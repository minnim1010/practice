...
#### 편집 가능 모드(Editable Mode)로 설치

`uv pip install -e .` 명령어는 현재 프로젝트를 "편집 가능 모드"로 가상 환경에 설치합니다.
- `-e`: "editable"을 의미합니다.
- `.`: 현재 디렉토리(프로젝트 루트)를 가리킵니다.

```bash
uv pip install -e .
```

편집 가능 모드로 설치하면, 프로젝트의 소스 코드를 수정할 때마다 패키지를 다시 설치할 필요가 없습니다. 변경 사항이 즉시 가상 환경에 반영되어 개발 생산성을 높여줍니다.

#### 개발용 의존성 포함하여 설치

`[project.optional-dependencies]`에 정의된 개발용 의존성(예: `dev`)을 포함하여 설치하려면 다음과 같이 실행합니다.
